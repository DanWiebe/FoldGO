...
* annot - from topGO manual: functions used to compile a list of GO terms such that each element in the list is a character vector containing all the gene identifiers that are mapped to the respective GO term.
* bggenes - vector contains background set of genes

```{r annot, warning = FALSE, message = FALSE, eval = FALSE}
library(topGO)
annotobj <- FuncAnnotGroupsTopGO(groups = up_groups, namespace = "MF", GO2genes = gaf_list, annot = topGO::annFUN.GO2genes, bggenes = bggenes)
```

#### Using annFUN.org db annotaions

Another possibility is to use bioсonductor packages containing annotations for specific organism. For example org.Hs.eg.db (Human) and org.Mm.eg.db (Mouse). 
